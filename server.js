...
client.on("message", async message => {
  ...
  if (command === "kick") {
    ...
    if (!message.member.roles.some(r => ["fondateur", "Admin", 'Modérateur'].includes(r.name)))
  ...
  if (command === "ban") {
    ...
    if (!message.member.roles.some(r => ["Admin", "fondateur"].includes(r.name)))
      return message.reply("Sorry, you don't have permissions to use this!");
  ...
  if (command === "wea") {
    let url = 'https://api.openweathermap.org/data/2.5/weather?q=' + args.join("-") + '&lang=fr' + '&units=metric' + '&appid=' + Wapi;
    url = url.replace(/ /g, '%20')
    url = url.replace(/\s/g, '%20')
    request.get({
      url: url,
